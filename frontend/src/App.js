...
const AdminPage = () => {
  ...
  const verifyToken = async (token) => {
    try {
      ...
      if (res.ok) { setIsAuthenticated(true); fetchPlaces(token); }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { localStorage.setItem('admin_token', data.token); setIsAuthenticated(true); fetchPlaces(data.token); toast.success('Connexion réussie !'); }
      else toast.error(data.detail || 'Mot de passe incorrect');
    } catch { toast.error('Erreur de connexion'); }
    finally { setLoading(false); }
  };

  const handleLogout = () => { localStorage.removeItem('admin_token'); setIsAuthenticated(false); setPlaces([]); toast.success('Déconnexion réussie'); };
